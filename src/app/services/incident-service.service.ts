...
import {
  IncidentData,
  IncidentStatsDTO,
} from '../models/incidentData.interface';
...
export class IncidentServiceService {
  ...
  private createBaseUrl: string =
    'http://localhost:7209/api/Incident/CreateIncident';

  getDataBasedOnStatus(employeeid: number, isUser: boolean): Observable<any> {
    return this.http.get<any>(
      `http://localhost:7209/api/Incident/GetIncidentsByEmployeeId/${employeeid}/${isUser}`
    );
  }
  ...
  public getSingleIncident(incidentId: number): Observable<IncidentData> {
    return this.http
      .get<IncidentData>(
        `http://localhost:7209/api/Incident/GetUserUpdateIncident/${incidentId}`
      )
  ...
  public getSingleFullIncident(incidentId: number): Observable<IncidentData> {
    return this.http
      .get<IncidentData>(
        `http://localhost:7209/api/Incident/GetIncident/${incidentId}`
      )
      .pipe(catchError(this.handleError));
  }
  ...
  public updateUserIncident(
    ...
    return this.http.put<any>(
      `http://localhost:7209/api/Incident/UserUpdateIncident/${incidentId}`,
      incident
    );
  }
  ...
  public getAssignedIncident(employeeId: number): Observable<IncidentData> {
    return this.http
      .get<IncidentData>(
        `http://localhost:7209/api/AssignedIncident/GetAssignedIncidentsForEmployee/${employeeId}`
      )
      .pipe(catchError(this.handleError));
  }
  ...
  public updateIncident(
    ...
    return this.http.put<IncidentData>(
      `http://localhost:7209/api/Incident/UpdateIncident/${incidentId}`,
      incident
    );
  }
  ...
  public deleteDraftIncidentById(incidentId: number): Observable<any> {
    return this.http.delete<any>(
      `http://localhost:7209/api/Incident/DeleteIncidentById/${incidentId}`
    );
  }
}
